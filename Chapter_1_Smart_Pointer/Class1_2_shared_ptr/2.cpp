...
struct ImuSensorData {
  ...
  // 自定义构造器(初始化列表)
  ImuSensorData(float ax = 0, float ay = 0, float az = 0,
                long long timestamp = 0)
      : ax(ax), ay(ay), az(az), timestamp(timestamp) {}
};
...
void task1_shared_ownership() {
  ...
  std::shared_ptr<ImuSensorData> pData =
      std::make_shared<ImuSensorData>(9.8, 0, 0, 100);
  ...
  std::cout << "Count before : " << pData.use_count()
            << std::endl;  // 调用前打印
  ...
  std::cout << "Count after : " << pData.use_count()
            << std::endl;  // 调用后打印
}
...
void task2_custom_deleter() {
  ...
  // [你的代码在这里]
  std::shared_ptr<void> buffer(raw_buffer,  // 传入原指针
                               [](void* p)  // lambda释放
                               {
                                 std::free(p);
                                 // p = nullptr; //
