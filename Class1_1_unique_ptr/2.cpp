...
// [你的代码在这里]
std::unique_ptr<ImuRawData> createImuData(float ax = 0, float ay = 0,
                                          float az = 0, float gx = 0,
...
int main() {
  ...
  // [你的代码在这里]
  std::unique_ptr<ImuRawData> pImu =
      createImuData(0.1, 0.2, 9.8);  // 生产者函数生产
  ...
  // TODO: 执行移动语义
  // [你的代码在这里]
  std::unique_ptr<ImuRawData> pImuMoved =
      std::move(pImu);  // 独占指针不可copy但可move
...
std::unique_ptr<ImuRawData> createImuData(float ax, float ay, float az,
                                          float gx, float gy, float gz,
                                          long long timestamp) {
  return std::make_unique<ImuRawData>(
      ImuRawData{ax, ay, az, gx, gy, gz, timestamp});
}
